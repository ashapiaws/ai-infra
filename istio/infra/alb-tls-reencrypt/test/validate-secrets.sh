#!/usr/bin/env bash
# Validate cross-namespace secret replication for ALB TLS Re-encryption Pattern
# Verifies TLS secrets exist in all target namespaces with matching content
set -euo pipefail

SECRET_NAME="${SECRET_NAME:-istio-ingress-tls}"
SOURCE_NAMESPACE="${SOURCE_NAMESPACE:-istio-system}"
TARGET_NAMESPACES="${TARGET_NAMESPACES:-default}"

echo "=== Secret Replication Validation: ALB TLS Re-encryption Pattern ==="
...
# Test 2: Verify secret type is kubernetes.io/tls
...
# Test 3: Get source secret hash for comparison
echo "[Test 3] Computing source secret hash..."
SOURCE_HASH=$(kubectl get secret "$SECRET_NAME" -n "$SOURCE_NAMESPACE" \
  -o jsonpath='{.data.tls\.crt}' 2>/dev/null | sha256sum | cut -d' ' -f1)
echo "  Source cert hash: ${SOURCE_HASH:0:16}..."

# Test 4: Verify secret exists in each target namespace with matching content
echo "[Test 4] Checking target namespaces..."
...
for NS in "${NAMESPACES[@]}"; do
  ...
  if [[ "$SOURCE_HASH" == "$TARGET_HASH" ]]; then
    echo "  PASS: Secret in '$NS' matches source (hash: ${TARGET_HASH:0:16}...)"
  else
    echo "  FAIL: Secret in '$NS' does NOT match source"
    echo "    Source: ${SOURCE_HASH:0:16}..."
    echo "    Target: ${TARGET_HASH:0:16}..."
    ALL_PASS=false
  fi
done
