# Gateway API - ALB Plaintext Pattern
...
ACM certificate and forwards plaintext HTTP traffic to the Istio Ingress Gateway
on port 8080.
...
4. **Istio Ingress Gateway** deployed with ClusterIP service type, listening on port 8080 (see `../istio-values.yaml`)
...
Client ──HTTPS──► ALB (TLS termination via ACM)
                    ...
                    ▼ HTTP (plaintext, backend-protocol: HTTP)
              Istio Ingress Gateway (ClusterIP:8080)
                    │  No TLS cert needed for this hop
                    ▼
...
|-----------|-------|---------|
| `backend-protocol` | HTTP | Plaintext to Istio (no re-encryption) |
...
# Test connectivity (replace with actual ALB DNS)
curl -v https://httpbin.example.com/headers
```
